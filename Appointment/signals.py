    ...
    if not instance.is_sms_sent:
        ...
        if instance.appointment_location == 'Online':
            ...
            PhoneMessage.objects.create(
                ...
                text = f"Online appointment confirmed: {patient.full_name} has an online appointment with Dr. {doctor.name} on {app_date} at {app_time}, \n\nThanks for using TraumaCare, a product by RedExpo."
            )
            ...
            PhoneMessage.objects.create(
                ...
                text = f"Appointment Confirmation: {patient.full_name} has an appointment with Dr. {doctor.name} at {hospital.name}, {location.name}, on {app_date} at {app_time}. \n\nThanks for using TraumaCare, a product by RedExpo."
            )
